        ...
        <div className="flex flex-col gap-8">
          ...
          <dl className="grid gap-6 sm:grid-cols-3">
            <div className="flex flex-col gap-1.5">
              <dt className="font-mono text-[10px] uppercase tracking-[0.3em] text-fg-dim">{t.mail}</dt>
              <dd className="m-0">
                <a href={email?.href} className="font-sans text-[15px] font-bold text-fg underline-offset-4 hover:underline">
                  {email?.value}
                </a>
              </dd>
            </div>
            <div className="flex flex-col gap-1.5">
              <dt className="font-mono text-[10px] uppercase tracking-[0.3em] text-fg-dim">{t.meet}</dt>
              <dd className="m-0 font-sans text-[15px] font-bold text-fg">{profile.location}</dd>
            </div>
            <div className="flex flex-col gap-1.5">
              <dt className="font-mono text-[10px] uppercase tracking-[0.3em] text-fg-dim">{t.find}</dt>
              <dd className="m-0 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[12px]">
                {socials.map((c) => (
                  <a
                    key={c.key}
                    href={c.href}
          ...
          <div className="flex flex-col gap-4">
            {formEnabled ? (
              ...
              <a
                href={email?.href}
                className="inline-flex min-h-[40px] w-fit items-center gap-2 rounded-full bg-fg px-5 font-mono text-[11px] uppercase tracking-[0.15em] text-bg transition-opacity hover:opacity-90"
              >
                {t.mail} →
              </a>
            )}
            {resume && (
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex min-h-[40px] w-fit items-center gap-2 rounded-full border border-fg/30 px-5 font-mono text-[11px] uppercase tracking-[0.15em] text-fg transition-colors hover:border-fg/70"
              >
